...
class _EmotionsQuizPageState extends State<EmotionsQuizPage> {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      ...
      // Call the API service method with proper error handling
      Map<String, dynamic>? result;
      try {
        result = await RestApiService.instance.getEmotionsQuiz(
        ...
        print('═══════════════════════════════════════');
        print('❌ Method call error: $methodError');
        print('   Error type: ${methodError.runtimeType}');
        print('   Method name: getEmotionsQuiz');
        print('   Service instance type: ${RestApiService.instance.runtimeType}');
        print('═══════════════════════════════════════');
        rethrow;
      }

      print('📥 Received result: ${result != null ? 'Yes' : 'No'}');
      if (result != null) {
        print('   Has questions: ${result['questions'] != null}');
        print('   Fallback flag: ${result['fallback']}');
        if (result['questions'] != null) {
          print('   Question count: ${(result['questions'] as List).length}');
        }
      }
      ...
      if (result != null &&
        ...
        print('📋 Processing ${questions.length} AI-generated questions...');
          ...
          if (mounted) {
            setState(() {
              ...
              _errorMessage = isTagalog
                  ? 'Walang questions na nakuha mula sa AI. Subukan ulit.'
                  : 'No questions received from AI. Please try again.';
            });
          }
          return;
        }
        ...
        // Move heavy processing off main thread using compute for better performance
        ...
        print('✅ Processed ${questions.length} questions successfully');
        ...
        print(
            '✅ Loaded ${_questions.length} AI-generated Emotions quiz questions');
        return;
      } else {
  ...
  void _shuffleOptions(Map<String, dynamic> question, [int questionIndex = 0]) {
    ...
    print(
        '   Q${questionIndex + 1}: Correct answer is at index: $newCorrectIndex (Letter: ${String.fromCharCode(65 + newCorrectIndex)})');
  }
  ...
  Widget build(BuildContext context) {
    // Use cached settings instead of looking up every build
    final isTagalog = _isTagalog;
            ...
            children: [
              ...
              Text(
                score == questions.length
                    ...
                    : score >= questions.length / 2
                        ? (isTagalog
                            ? 'Magaling! Mahusay na pag-unawa sa emosyon!'
                            : 'Great! Good understanding of emotions!')
                        : (isTagalog
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          isTagalog
              ? 'Quiz tungkol sa Emosyon - ${widget.difficulty == 'easy' ? 'Madali' : widget.difficulty == 'medium' ? 'Katamtaman' : 'Mahirap'}'
              : 'Emotions Quiz - ${widget.difficulty[0].toUpperCase()}${widget.difficulty.substring(1)}',
          style: TextStyle(
            color: primaryTextColor,
            fontWeight: FontWeight.bold,
            fontSize: 16,
          ...
          children: [
            ...
            ClipRRect(
              ...
              child: LinearProgressIndicator(
                ...
                valueColor:
                    const AlwaysStoppedAnimation<Color>(Color(0xFF005BEA)),
              ),
            ),
            const SizedBox(height: 32),
            // Question
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final correctIndex = currentQuestion['correct'] as int? ?? 0;
